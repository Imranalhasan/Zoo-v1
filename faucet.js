...
class DuckChainFaucet {
  ...
  async main() {
    const dataFile = path.join(__dirname, "authFaucet.txt");
    const walletFile = path.join(__dirname, "wallets.txt");

    const authorizations = fs.readFileSync(dataFile, "utf8").replace(/\r/g, "").split("\n").filter(Boolean);

    const wallets = fs.readFileSync(walletFile, "utf8").replace(/\r/g, "").split("\n").filter(Boolean);
